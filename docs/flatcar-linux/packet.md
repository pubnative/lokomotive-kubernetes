...
| reservation_ids_default | Specify a default reservation ID for nodes not listed in the `reservation_ids` map. An empty string means "use no hardware reservation". `next-available` will choose any reservation that matches the worker pool's device type and facility | "" | "next-available"|
