...
* Running libvirtd system service, see [libvirt Setup]
...
Add the [terraform-provider-ct](https://github.com/poseidon/terraform-provider-ct) plugin binary for your system to `~/.terraform.d/plugins/`, noting the `_v0.3.1` suffix.
...
Build terraform-provider-libvirt from source because pool definition feature was not released yet.
This means not only libvirt should be installed but also the development packets.

```sh
$ sudo dnf install libvirt-devel  # or sudo apt install libvirt-dev
$ go get github.com/dmacvicar/terraform-provider-libvirt
$ cd $GOPATH/src/github.com/dmacvicar/terraform-provider-libvirt
$ GO111MODULE=on GOFLAGS=-mod=vendor make install
...
provider "libvirt" {
  # TODO: add version once released
  uri = "qemu:///system"
  alias = "default"
}
```
...
Create a file called `mycluster.tf` with these contents but remember to change the `CHANGEME` values:
...
module "controller" {
  ...
  os_image_unpacked = "file:///home/CHANGEME/flatcar_production_qemu_image.img"

  ssh_keys = [
    "ssh-rsa AAAAB3Nza...",
  ]

  asset_dir = "/home/CHANGEME/clusters/asset"
...
Now download the Flatcar image. The example below uses the "Edge" Flatcar channel.
...
$ wget https://edge.release.flatcar-linux.net/amd64-usr/2135.99.0/flatcar_production_qemu_image.img.bz2
$ bunzip2 flatcar_production_qemu_image.img.bz2
$ qemu-img resize flatcar_production_qemu_image.img +5G
```

Now check the contents of `mycluster.tf`.
Change at least the `ssh_keys` entry to match the content of your `~/.ssh/id_KEY.pub` and
change the `os_image_unpacked` path to point to the image location from above.
...
Add your SSH private key to `ssh-agent` so that it's unlocked if secured with a passphrase
(you can skip this if the key way created with the GNOME Passwords and Keys tool).
...
In 2-4 minutes, the Kubernetes cluster will be ready, depending on your downlink speed and system performance.
...
```
$ export KUBECONFIG=/home/CHANGEME/clusters/asset/auth/kubeconfig  # matching the asset_dir from the config file
$ kubectl get nodes
